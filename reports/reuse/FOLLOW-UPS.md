...
| FU-T3-01 | `crates/sem_os_policy/src/service.rs:1324,1458`, `enforce.rs:64,67`, `authoring/governance_verbs.rs:379-560` | low | SemOS object/changeset ids are still `Uuid::new_v4()`; `sem-os-id` now exists but the core has not been switched to it (out of T3's "new crates" scope; switching changes id ordering semantics and needs its own tranche). | DSL-T5: adopt `TimeOrderedGenerator` behind the existing ports. |
| FU-T3-02 | `crates/semantic-pack/src/source.rs` `SegmentPermission::required_by` | low | The action-class → permission mapping is a fixed generic table in the core. If a domain needs a different mapping it must be made pack data (`capability_segments.action_map`). | Only if a second domain needs it. |
