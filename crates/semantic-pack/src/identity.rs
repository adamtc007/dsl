...
semantic_id!(PolicyContextId, "policy context id");
semantic_id!(PolicyAttributeId, "policy attribute id");
semantic_id!(PrivilegeId, "privilege id");

fn validate_selector_text(
    kind: &'static str,
    value: &str,
    ...
    if value.starts_with('.')
        ...
        || value.contains(".-")
        || value.contains("-.")
        || (!allow_trailing_separator && (value.ends_with('.') || value.ends_with('-')))
    {
        return Err(invalid("contains an empty or malformed segment"));
    }
    if !value.bytes().all(|byte| {
...
macro_rules! selector_text {
    ($name:ident, $kind:literal, $allow_trailing_separator:literal) => {
        #[doc = concat!("Validated ", $kind, ".")]
        #[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
        pub struct $name(String);
        ...
        impl $name {
            ...
            pub fn new(value: impl Into<String>) -> Result<Self, IdentityError> {
                let value = value.into();
                validate_selector_text($kind, &value, $allow_trailing_separator)?;
                Ok(Self(value))
            }
